sainte_lague = function(..., okreg, frekwencja = 100){
  ...
  dane = data.frame(dzielnik = seq(from = 1, to = 1 + (okregi[okreg, 2] * 2), by = 2))
  ...
  wynik_sl = data.frame("Komitet" = paste(cyfry_rzymskie[1:length(wyniki)], "Komitet"),
                        "Sainte-Lague" = macierz_dane)
  wynik_sl
}
